...
Au final, vous deverez récupérer la position du curseur par rapport à l'élément, modisier les variables CSS `--mouse-x` et `--mouse-y` en fonction de cette position.
